...
//   FutureOr<void> quantityConfirmButtonClicked(QuantityConfirmButtonClicked event, Emitter<AddPurchaseState> emit)async {
//    CollectionReference reference=firestore.collection('UesrData').doc(user!.uid).collection('records');
//    await reference.add('purcaseItems':event.purchasedItem.);
//   }
  ...
  FutureOr<void> addRecordConfirm(AddRecordConfirm event, Emitter<AddPurchaseState> emit) async{
    try{
    emit(PurchaseRecordAddLoading());
    CollectionReference reference=firestore.collection('UserData').doc(user!.uid).collection('PurchaseRecords');
